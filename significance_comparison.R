...
lm_aov_sumsqs_rils<-fread('Data/lm_aov_sumsqs.rils.csv')
lm_aov_sumsqs_rils$V1<-NULL
lm_aov_sumsqs_parents<-fread('Data/lm_aov_sumsqs_parents.csv')
...
pdf("Figures/significancetests_rils.pdf")
melt(rilprops)%>%
...
pdf("Figures/significancetests_parents.pdf")
melt(parentprops)%>%
  ggplot(aes(variable,value))+
  #geom_point()+
  geom_violin()+
